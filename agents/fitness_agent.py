...
from plugins.render_template_plugin import RenderTemplatePlugin
from semantic_kernel.agents import ChatCompletionAgent
load_dotenv()
AGENT_ID = "asst_qd9bqmxFKPs1i0PA2wZgMqxG"

async def create_fitness_agent(kernel):
    # creds = DefaultAzureCredential()
    # client = AzureAIAgent.create_client(credential=creds)
    # fitness_plugin = FitnessPlugin()
    # templete_plugin = RenderTemplatePlugin()
    # agent_definition = await client.agents.get_agent(agent_id=AGENT_ID)
    # agent = AzureAIAgent(client=client, definition=agent_definition, plugins=[fitness_plugin, templete_plugin])
    ...
    return ChatCompletionAgent(
        kernel=kernel,
        name="Fitness",
        plugins = [fitness_plugin, templete_plugin],
        instructions="""
